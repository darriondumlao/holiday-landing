...
const YearbookModal = dynamic(() => import('@/components/YearbookModal'))
    ...
    <div className="fixed inset-0 overflow-hidden bg-black">
      ...
      {showQuestionLightbox && (
        ...
        >
          ...
          <div className={`relative z-10 w-full max-w-md px-6 flex flex-col items-center space-y-6 transition-transform duration-300 ${
      ...
      >
          {/* Desktop: Fixed "prompt" side button on left edge — matches mobile style */}
          <button
            onClick={openQuestionLightbox}
            className='hidden md:block fixed left-0 top-1/2 -translate-y-1/2 z-50 transition-all duration-300 cursor-pointer'
          >
            <div className='bg-white rounded-r-lg py-4 px-1.5 shadow-xl flex flex-col items-center gap-1'>
              <span
                className='text-black text-[10px] font-bold tracking-wider'
                style={{ writingMode: 'vertical-rl', textOrientation: 'mixed' }}
              >
          ...
          {/* Layout: full-width on mobile, full-width on desktop */}
          <div className='flex flex-col w-full h-full'>
            ...
            <div className='h-full'>
              ...
              <div className='md:hidden w-full h-full overflow-y-auto overscroll-y-contain scrollbar-hide'>
                <div className='flex flex-col gap-3 w-full max-w-[360px] mx-auto px-4 pb-20 pt-1'>
                  ...
                  {showYearbookModal && (
                    <div className='w-full'>
                      <StaticModalWrapper className='w-full'>
                        <YearbookModal onClose={() => setShowYearbookModal(false)} />
                      </StaticModalWrapper>
                    </div>
                  )}
                </div>
              ...
              <div className='hidden md:block h-full w-full overflow-y-auto px-8 py-6 scrollbar-none'>
                <div className='grid grid-cols-4 gap-5 max-w-[1520px] mx-auto'>
                  {showDownloadModal && downloadModalData && (
                    <StaticModalWrapper className='w-full'>
                      <BottomLeftModal
                        title={downloadModalData.title}
